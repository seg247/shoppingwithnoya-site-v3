...
describe('filterAndSort', () => {
  it('filters by category', () => {
    const result = filterAndSort(mockDeals, { category: 'Electronics', sort: 'latest' });
    expect(result).toHaveLength(2);
  });

  it('filters fire deals (includes FIRE and HOT)', () => {
    const result = filterAndSort(mockDeals, { category: 'fire', sort: 'latest' });
    expect(result).toHaveLength(2);
    expect(result.every(d => d.rating === 'FIRE' || d.rating === 'HOT')).toBe(true);
  });

  it('sorts by discount', () => {
    const result = filterAndSort(mockDeals, { category: 'all', sort: 'discount' });
    expect(result[0].discount).toBe(50);
  });

  it('sorts by latest', () => {
    const result = filterAndSort(mockDeals, { category: 'all', sort: 'latest' });
